...
import PrentPolicyPount from '../PrentPolicyPount';
  ...
  render() {
    ...
    return (
      <Spin spinning={this.props.loading} size="large" tip="加载中...">
		<PrentPolicyPount indexCode="666101" nextBranch={this.state.nextBranch}/>
        <Card loading={this.props.loading} title={this.props.title}>
        <div className={styles.report}>
          <Alert style={{ marginBottom: '16px' }}  style={{display:'none'}}message={
            <div>
          ...
          <Alert style={{ marginBottom: '16px' }}  message={
            <Breadcrumb>
            {
              this.state.location && this.state.location.map((l,index)=>
                ...
                console.log(this.state.nextBranch)
                this.props.dispatch({
                    type: 'report1/setParams',
                    payload: {
                      reportId: this.props.code,
...
export default connect(({ report1 ,user}, { code }) => ({
  user,
  params: report1.params[code],
  data: report1.data[code],
  loading: report1.loading[code],
}))(Report)
